...
#if defined(CONFIG_LGE_DISPLAY_COMMON)
static void dp_panel_skip_aux_recfg(struct dp_panel *dp_panel, bool skip)
{
	struct dp_panel_private *panel;
	...
	panel->skip_aux_recfg = skip;
	DP_INFO("[OK] (%d)\n", panel->skip_aux_recfg);
}
	...
	struct drm_connector *connector)
{
	...
	while (!panel->skip_aux_recfg && count++ <= 30) {
...
#if defined(CONFIG_LGE_DISPLAY_COMMON)
	dp_panel->skip_aux_recfg = dp_panel_skip_aux_recfg;
#endif
