...
static int dp_display_process_hpd_high(struct dp_display_private *dp)
{
	...
	if (dp_display_state_is(DP_STATE_CONNECTED) &&
		...
		dp->panel->skip_aux_recfg(dp->panel, true);
	} else
		dp->panel->skip_aux_recfg(dp->panel, false);
#endif
...
static int dp_display_prepare(struct dp_display *dp_display, void *panel)
{
	...
	if (is_ds_connected()) {
		if ((dp->aux->state & DP_STATE_TRAIN_1_SUCCEEDED) &&
			...
			DP_INFO("LINK_TRAIN_1 / LINK_TRAIN_2 succeeded\n");
		} else {
			DP_ERR("%s failed, aux state: %d\n", __func__, dp->aux->state);
			return -EINVAL;
		}
	}
#endif
...
bool is_dd_powermode()
{
	...
	if (IS_ERR_OR_NULL(dp)) {
		...
		/* TODO : It seem that dp->power_on is used on SM8250 MSM DP
		ret = dp->power_on;
		*/
	}
	return ret;
