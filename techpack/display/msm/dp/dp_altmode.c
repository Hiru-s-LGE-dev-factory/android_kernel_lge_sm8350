...
#if defined(CONFIG_LGE_DISPLAY_COMMON)
static void dp_altmode_update_vid(u8 id1, u8 id2, bool connected)
{
	u16 vid = (((id2 & 0xFF) << 8)| (id1 & 0xFF));
	DP_INFO("[%s] set vid=0x%08x\n", (!connected ? "CONNECT" : "DISCONNECT"), vid);

	lge_dp_set_id((unsigned int)vid);
}
#endif
...
#if defined(CONFIG_LGE_DISPLAY_COMMON)
			dp_altmode_update_vid(payload[4], payload[5], altmode->connected);
#endif
