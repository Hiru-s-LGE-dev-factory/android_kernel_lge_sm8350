...
int lge_cmdline_bootup_resolution = 0;
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_COMMON)
	if ((display->panel->lge.use_fps_switch) &&
			(display->panel->lge.ddic_ops) &&
			(display->panel->lge.ddic_ops->set_fps_cmds)) {
		if(timing.refresh_rate_div > 0)
			DSI_INFO("Faking %d fps", timing.refresh_rate / timing.refresh_rate_div);
		display->panel->lge.ddic_ops->set_fps_cmds(display->panel, timing.refresh_rate, false);
	}
#endif
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_COMMON)
static int __init lge_display_check_disable_ecc(char *status) __attribute__((unused));
...
static int __init lge_display_check_bootup_resolution(char *res) __attribute__((unused));
static int __init lge_display_check_bootup_resolution(char *res)
{
	long value = 0;
	value = simple_strtol(res, NULL, 10);
	lge_cmdline_bootup_resolution = (uint32_t)value;
	pr_info("[Display] resolution=%d, cmdline=%s\n", lge_cmdline_bootup_resolution, res);

	return 1;
}
__setup("androidboot.vendor.lge.init_res=", lge_display_check_bootup_resolution);
...
int dsi_display_post_kickoff(struct dsi_display *display)
{
	...
	if (c_conn->last_panel_power_mode == SDE_MODE_DPMS_ON) {
		...
		if (display->panel->lge.use_exscreen_control &&
				display->panel->lge.expandable_pending &&
				(display->panel->lge.ddic_ops &&
				 display->panel->lge.ddic_ops->set_hl_mode)) {
			pr_info("kickoff hl_mode pending value\n");
			display->panel->lge.ddic_ops->set_hl_mode(display->panel,
					display->panel->lge.expandable_pending_mode);
			display->panel->lge.expandable_pending = false;
		}
