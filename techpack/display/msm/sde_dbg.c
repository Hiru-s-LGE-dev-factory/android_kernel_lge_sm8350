...
#define LGE_SDE_FILED_LOG_LEVEL 0

#if LGE_SDE_FILED_LOG_LEVEL
...
#if (CONFIG_LOG_BUF_SHIFT == 22)
#ifdef DEFAULT_DBGBUS_VBIFRT
...
struct sde_debug_bus_entry {
	...
	void (*analyzer)(void __iomem *mem_base,
				...
				u32 block_id_off, u32 test_id_off);
};
		...
		u32 test_id_off)
{
	dev_err(sde_dbg_base.dev, "xbar 0x%x %d %d 0x%x\n",
			entry->wr_addr, entry->block_id + block_id_off,
			entry->test_id + test_id_off, val);
}
		...
		bool in_mem)
{
	...
	for (i = 0; i < head->block_id_max; i++) {
		for (j = 0; j < head->test_id_max; j++) {
...
static void _sde_dbg_dump_sde_dbg_bus(struct sde_dbg_sde_debug_bus *bus)
{
	...
	if (!dump_mem || !bus->entries || !bus_size)
		return;
	/* allocate memory for each test id */
	for (i = 0; i < bus_size; i++) {
		head = bus->entries + i;
		list_size += (head->block_id_max * head->test_id_max);
	}
