...
extern void    cam_ois_create_sysfs(void);
extern void    cam_ois_destroy_sysfs(void);
extern int32_t cam_ois_init(struct cam_ois_ctrl_t *o_ctrl);
uint8_t        zdrift_ready;
uint8_t        cam_power_up_cnt = 0;
#endif
#ifdef CONFIG_MACH_LGE
extern int lge_ois_parse_userdata(struct cam_cmd_ois_userdata *ois_userdata, struct cam_ois_ctrl_t *o_ctrl);
...
#ifdef CONFIG_LGE_CAMERA
	if (o_ctrl->is_ois_aat == 1)
		cam_ois_create_sysfs();

	if (o_ctrl->io_master_info.cci_client->sid == 0x1C >> 1)
		zdrift_ready = 0;

	cam_power_up_cnt++;
	CAM_DBG(CAM_OIS, "cam_power_up_cnt: %d", cam_power_up_cnt);
#endif
	return rc;
cci_failure:
...
#ifdef CONFIG_LGE_CAMERA
		o_ctrl->is_ois_aat = ois_info->is_ois_aat;
#endif
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
...
#ifdef CONFIG_MACH_LGE
	struct cam_cmd_ois_userdata *ois_userdata = NULL;
#endif
...
#ifdef CONFIG_LGE_CAMERA
        CAM_DBG(CAM_OIS, "before sensor_complete cam_power_up_cnt: %d", cam_power_up_cnt);
        if (cam_power_up_cnt >= 1) {
            cam_power_up_cnt = 0;
            CAM_DBG(CAM_OIS, "skip sensor_complete cam_power_up_cnt: %d", cam_power_up_cnt);
        } else {
            ...
            CAM_DBG(CAM_OIS, "after sensor_complete cam_power_up_cnt: %d", cam_power_up_cnt);
        }
#endif
    ...
    case CAM_OIS_PACKET_OPCODE_OIS_USERDATA:
        ...
        CAM_ERR(CAM_OIS, "call parse_ois_userdata");
		lge_ois_parse_userdata(ois_userdata, o_ctrl);
		break;
#endif
