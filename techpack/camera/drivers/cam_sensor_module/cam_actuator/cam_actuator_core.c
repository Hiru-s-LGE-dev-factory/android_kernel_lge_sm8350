...
extern void write_z_value(struct i2c_settings_list *i2c_list);
#endif
#ifdef CONFIG_MACH_LGE
extern bool lge_actuator_data_enqueue(uint32_t reg_addr, uint32_t reg_data, struct cam_actuator_ctrl_t *a_ctrl);
extern int lge_actuator_parse_userdata(struct cam_cmd_actuator_userdata *af_userdata, struct cam_actuator_ctrl_t *a_ctrl);
	...
	struct i2c_settings_list *i2c_list)
{
	...
	if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
...
#ifdef CONFIG_MACH_LGE
        {
            ...
            if (a_ctrl->timer == NULL)
            {
                lge_actuator_data_enqueue(i2c_list->i2c_settings.reg_setting[0].reg_addr,
	...
	void *arg)
{
...
#ifdef CONFIG_MACH_LGE
    struct cam_cmd_actuator_userdata *actuator_userdata = NULL;
#endif
	if (!a_ctrl || !arg) {
		CAM_ERR(CAM_ACTUATOR, "Invalid Args");
		return -EINVAL;
    ...
    if ((csl_packet->header.op_code & 0xFFFFFF) ==
         CAM_ACTUATOR_PACKET_OPCODE_ACTUATOR_USERDATA)
    {
        CAM_INFO(CAM_ACTUATOR, "CAM_ACTUATOR_PACKET_OPCODE_ACTUATOR_USERDATA Opcode: %d",
                    csl_packet->header.op_code & 0xFFFFFF);

        if(a_ctrl->cam_act_state < CAM_ACTUATOR_CONFIG) {
            rc = -EINVAL;
            CAM_ERR(CAM_ACTUATOR,
                    "Not in right state to parse userdata ACTUATOR: %d",
        ...
        if (!total_cmd_buf_in_bytes) {
            CAM_ERR(CAM_ACTUATOR, "No total cmd buf bytes !!");
             goto end;
        }
        ...
        actuator_userdata = (struct cam_cmd_actuator_userdata *)cmd_buf;
        lge_actuator_parse_userdata(actuator_userdata, a_ctrl);
