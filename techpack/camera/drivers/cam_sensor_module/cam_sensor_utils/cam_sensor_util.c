	...
	for (i = 0; i < power_setting_size; i++) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (j == num_vreg)
				power_setting[i].seq_val = INVALID_VREG;
			break;
//SHMOON_201006_BEGIN : LDAF Bringup
		case SENSOR_VIO_LDAF:
			for (j = 0; j < num_vreg; j++) {

				if (!strcmp(soc_info->rgltr_name[j],
					"ldaf_vio")) {
					CAM_DBG(CAM_SENSOR,
						"i: %d j: %d ldaf_vio", i, j);
					power_setting[i].seq_val = j;
			...
			if (j == num_vreg)
				power_setting[i].seq_val = INVALID_VREG;
			break;
		case SENSOR_VDD_LDAF:
			for (j = 0; j < num_vreg; j++) {

				if (!strcmp(soc_info->rgltr_name[j],
					"ldaf_vdd")) {
					CAM_DBG(CAM_SENSOR,
						"i: %d j: %d ldaf_vdd", i, j);
					power_setting[i].seq_val = j;
			...
			if (j == num_vreg)
				power_setting[i].seq_val = INVALID_VREG;
			break;
//SHMOON_201006_END : LDAF Bringup
		default:
			break;
		}
	}
