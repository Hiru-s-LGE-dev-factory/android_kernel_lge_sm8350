	...
	struct cam_isp_resource_node    *res)
{
	...
	struct cam_hw_soc_info                    *soc_info;
	struct cam_ife_csid_cid_data              *cid_data;
	/* LGE_CHANGE, CST, check regulator is enabled */
	int i;
	int rgltr_enabled = 1;
	...
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
		csid_reg->csi2_reg->csid_csi2_rx_cfg1_addr);
#else 	/* LGE_CHANGE, CST, check regulator is enabled */
	for (i=0; i< soc_info->num_rgltr; i++) {
		rgltr_enabled &= regulator_is_enabled(soc_info->rgltr[i]);
		if (!rgltr_enabled)
			CAM_ERR(CAM_ISP, "CSID:%d rgltr idx %d %s disabled.",
			csid_hw->hw_intf->hw_idx, i, soc_info->rgltr_name[i]);
	}
	if (rgltr_enabled) {
		cam_ife_csid_csi2_irq_ctrl(csid_hw, false);
