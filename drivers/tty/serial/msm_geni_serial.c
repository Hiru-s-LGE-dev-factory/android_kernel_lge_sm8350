...
extern int debug_accessory_detected;
#endif
...
#ifdef CONFIG_LGE_USB_DEBUGGER
	if (debug_accessory_detected == -1) {
		if(lge_get_usb_debugger() <= 0 && uart_console(uport))
			return;
	} else {
...
{
	if(uart_key_press_status != POW_KEY_PRESSED && status == UART_CONSOLE_ENABLED) {
		pr_info("geni_uart_console_status : power key is not pressed\n");
		if(lge_get_usb_debugger() <= 0)
			return;
	}
...
int msm_geni_serial_set_uart_console(int enable)
{
	...
	if(uart_key_press_status != POW_KEY_PRESSED && enable) {
		pr_err("Power key is not pressed. Block uart resume\n");
		if(lge_get_usb_debugger() <= 0)
			return ret;
	}
...
static int __init msm_geni_serial_init(void)
{
	...
	if(g_ms_status != UART_CONSOLE_ENABLED)
	{
		...
		schedule_delayed_work(&uart_console_work,msecs_to_jiffies(60*1000));//wait for console service start
	}
#endif
